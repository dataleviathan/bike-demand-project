...
dim(citibike) str(citibike) colnames(citibike) head(citibike) tail(citibike)
...
summary(citibike$Age) summary(citibike$TripDurationMinutes) summary(citibike$StartPerCapitaIncome) summary(citibike$EndPerCapitaIncome) summary(citibike$StartPctHouseholdsNoVehicle) summary(citibike$EndPctHouseholdsNoVehicle)

#Compute frequency

table(citibike$UserType) table(citibike$Gender)

#Plot the number of trips

ggplot(citibike, aes(x = DayOfWeek,)) + geom_bar() + geom_bar(colour = "white", fill = "dodgerblue") + ggtitle("Number of Trips by Day of the Week") + ylab("Trips") + xlab("Day of Week")

ggplot(citibike, aes(x = UserType, , fill = UserType)) + geom_bar() + geom_bar(colour = "white") + ggtitle("Number of Trips by User Type") + ylab("Trips") + xlab("User Type")

ggplot(citibike, aes(x = Age)) + geom_bar() + geom_bar( colour = "dodgerblue", fill = "dodgerblue") + ggtitle("Number of Trips by Age") + ylab("Trips") + xlab("Age")

ggplot(citibike, aes(x = Gender, fill = Gender)) + geom_bar() + ggtitle("Number of Trips by Gender") + ylab("Trips") + xlab("Gender")

ggplot(citibike, aes(x = Temperature)) + geom_bar()+ geom_bar( colour = "dodgerblue", fill = "dodgerblue") + ggtitle("Number of Trips by Temperature") + ylab("Trips") + xlab("Temperature")

#Missing data

sum(is.na(citibike)) sum(is.na(citibike$Age)) colSums(is.na(citibike))
...
summary(citibike$Temperature) summary(citibike$DailySnowFall) summary(citibike$DailyPrecipitation) summary(citibike$TripDurationMinutes) summary(citibike$DistanceMiles)

#Mean for Temperature is 62.72 degrees #Mean for DailySnowFall is 0.0903 inches #Mean for DailyPrecipitation is 0.0832 inches #Mean for TripDurationMinutes is 12.97 minutes #Mean for DistanceMiles is 1.12 miles

dim(citibikeDemand) str(citibikeDemand) colnames(citibikeDemand) head(citibikeDemand) tail(citibikeDemand)

sum(is.na(citibikeDemand)) sum(is.na(citibikeDemand$Demand))

#Identify Demand Patterns

mean(citibikeDemand$Demand) sd(citibikeDemand$Demand)

cor(citibikeDemand$Demand, citibikeDemand$TripDurationMinutes) cor(citibikeDemand$Demand, citibikeDemand$DistanceMiles)

#Mean and SD for predictive variables

mean(citibikeDemand$Demand) sd(citibikeDemand$Demand) mean(citibikeDemand$TripDurationMinutes) sd(citibikeDemand$TripDurationMinutes) mean(citibikeDemand$DistanceMiles) sd(citibikeDemand$DistanceMiles)

#Propability Table #2-Way Frequency Table #First argument will be rows, second argument will be columns
...
bin_width <- 2 * IQR(citibikeDemand$Demand) / length(citibikeDemand$Demand)^(1/3) ggplot(citibikeDemand, aes(x = Demand)) + geom_histogram(binwidth = bin_width) + geom_histogram( colour = "white", fill = "dodgerblue") + ggtitle("Distribution of Demand")

#Bar Plot

ggplot(citibikeDemand, aes(x = Demand, y = TripDurationMinutes)) + geom_bar(stat = "identity", fill="dodgerblue", color="dodgerblue") + ggtitle("Trip Duration per Demand Levels")

#Scatterplots

ggplot(citibikeDemand, mapping = aes(x = Demand, y = StartPerCapitaIncome )) + geom_point() + geom_point( colour = "dodgerblue") + ggtitle("Neighborhood Income per Demand Levels") + ylab("StartPerCapitaIncome") + xlab("Demand")

ggplot(citibikeDemand, mapping = aes(x = Demand, y = DistanceMiles)) + geom_point() + geom_point( colour = "dodgerblue") + ggtitle("Distance Traveled per Demand Levels") + ylab("Distance in Miles") + xlab("Demand")
